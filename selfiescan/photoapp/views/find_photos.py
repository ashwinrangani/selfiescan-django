...
from photoapp.utils.concurrency import try_acquire_slot, release_slot
...
def load_resize_for_rekognition_selfie(file_obj, max_side=1600):
...
def find_matches(selfie_file, event_id, tolerance=0.55):
    image_bytes = load_resize_for_rekognition_selfie(selfie_file)
...
def get_variant_urls(photo, event):
    ...
    if event.branding_enabled and photo.is_branded and photo.branded_image:
        image_for_download = photo.branded_image
        return {
            ...
            "download": generate_presigned_download(image_for_download),
        }

    image_for_download = photo.large_image if photo.large_image else photo.image
    return {
...
def process_selfie(request, event_id):
    ...
    if not event.is_find_photos_enabled:
        return render(request, "feature_disabled.html", {
        "message": "Photo search is currently disabled for this event."
    })
    ...
    if request.method == 'POST':
        if not try_acquire_slot():
            print("Please wait for a while, server is busy. Try again shortly.")
            return JsonResponse({
            "message": "Please wait for a while, server is busy. Try again shortly."
        }, status=429)
        
        #if number_of_photos < 1:
            #return JsonResponse({'message': 'Event Photos Not Uploaded Yet! Wait While Photos Are Ready.'})
        ...
        if selfie:
            print("Selfie found:", selfie.name, selfie.size)
            ...
            try:
                matching_images = find_matches(selfie, event_id)
                ...
                for photo_id, distance in matching_images:
                    ...
                    if not photo:
                        logger.warning(f"Matched photo {photo_id} not found in DB, skipping")
                        continue
                ...
                return JsonResponse({
                    'message': 'Matching photos found' if matches else 'Matching photos not found',
                    'matches': matches
                })
